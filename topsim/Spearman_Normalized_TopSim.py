...
def levenshtein_distance_process(form_set):
    normalized_form_distance_list = []

    # 全てのペアを生成
    pairs = combinations(form_set, 2)

    # 各ペアに対して normalized_levenshtein_distance_ability を計算
    for form1, form2 in pairs:
        distance = normalized_levenshtein_distance_ability(form1, form2)
        normalized_form_distance_list.append(distance)
    return normalized_form_distance_list

def Spearman_Normalized_TopSim(rule_set):
    ...
    normalized_form_distance_list = levenshtein_distance_process(form_set)
    spearman_normalized_topsim_values, spearman_normalized_p_value = spearmanr(hamming_distance_list, normalized_form_distance_list)
    return spearman_normalized_topsim_values, spearman_normalized_p_value